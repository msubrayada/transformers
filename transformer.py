...
from tensorflow.keras.layers import Layer, Conv1D, Dense, concatenate, Input, add, Dropout, LayerNormalization, MultiHeadAttention, Embedding
...
class Encoder(Layer):
    def __init__(self, n, embed_dim, max_length, num_heads, ff_dim, rate=0.1, pos_encoding=False, **kwargs):
    ...
    def get_config(self):
        ...
        config.update({"n": self.n, "embed_dim":self.embed_dim, "max_length": self.max_length, "num_heads":self.n_h, "ff_dim":self.f_d, "pos_encoding":self.pos_encoding, "rate":self.rate})
        return config
...
def get_transformer(n=6, embed_dim=300, max_length=49, roi_dim=512, roi_num=30, num_heads=6, ff_dim=512, vocab_size=10000):
    """
    n: number of enconcoders/decoders
